...
class CityPreprocessor:
    ...
    def load_and_preprocess_data(self):
        ...
        cities['distance_km'] = cities.apply(
            lambda row: self.dc.haversine_distance_in_km(self.london_lat, self.london_lng, row['lat'], row['lng']),
            axis=1
        )

        df = cities.sort_values('distance_km').rename(columns={'id': 'city_id', 'city': 'city_name'}).reset_index(
            drop=True
        )
...
class DistanceCalculator:
    ...
    def haversine_distance_in_km(self,lat1, lon1, lat2, lon2):
        ...
        of a sphere using their latitudes and longitudes. The result is the great-circle distance,
        which represents the distance along the surface of the sphere.
        """
...
class CostCalculator:
    ...
    def check_population(self,population):
        ...
        This function can be used to categorize populations into two groups based on a threshold.
        """
    ...
    def check_country(self,current_city_country,destination_country):
        ...
        This function can be used to determine if the current city and destination are in different countries.
        """
    ...
    def calculate_cost(self,current_city_country,current_city_id,df):
        ...
        - df (pandas.DataFrame): A DataFrame containing information about destination cities.
