...
class NetflixQuiz:
    ...
    def generate_genre_question(self):
        ...
        all_genres = ['Horror', 'Comedies', 'Romantic Movies', 'Documentaries', 'Reality TV', 'Anime Features']
    
        target_genre = np.random.choice(all_genres)        
        question = f"Which of the following belongs to the genre '{target_genre}'?"

        genre_data = self.data[self.data['listed_in'].str.contains(target_genre)]   #dataframe
        correct_opt = genre_data.sample()['title'].iloc[0]

        non_genre_data = self.data[~self.data['listed_in'].str.contains(target_genre)]
        incorrect_opts = non_genre_data.sample(n=3)['title'].tolist()   #convert the pandas Series into a Python list
    ...
    def generate_rating_question(self):
        ...
        correct_answers_all = self.data[self.data['rating'] == 'TV-MA']['title'].tolist()    #list of titles
        incorrect_opts = self.data[self.data['rating'].isin(['TV-Y','TV-Y7'])].sample(n=3)['title'].tolist()
    ...
    def generate_cast_question(self):
        ...
        incorrect_opts_all=set(self.data['cast'].str.split(',').explode().str.strip())-set(cast_list)
    ...
    def generate_question(self, difficulty):
        question_types_medium = [self.generate_rating_question, self.generate_country_question]
